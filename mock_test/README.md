...
- **Remote**: Path on SFTP server (for remote file deletion testing)
...
- Test SFTP remote file deletion (when configured)
...
| `sftp_helper.sh` | SFTP operations for remote file testing |
| `run_test_and_verify.sh` | Runs test and verifies against CSV |
| `verify_test_results.sh` | CSV-based verification script (local + remote) |
...
| `.env.test` | Test configuration (paths, settings, SFTP) |
...
The CSV rubric defines 86 test cases covering:
...
- ✅ SFTP remote file deletion (when SFTP server configured)
...
6. **SFTP testing is optional** - configure SFTP credentials in `.env.test` to test remote file deletion, or leave unconfigured for local-only testing
